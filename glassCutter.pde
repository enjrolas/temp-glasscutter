...
class Slicer {
  ...
  float displayScalingFactor, scalingFactor;
  Vertex min, max;
  PVector bound;
  Vertex boundVertex;
  char axis='x';
  ...
  {    
    ...
    for (thickSlice=0;thickSlice<slices.size()/microslices;thickSlice++)
    {
      ...
      for (slice=thickSlice*microslices;(slice<((thickSlice+1)*microslices)+(int)(overlap*microslices))&&(slice<slicer.slices.size());slice++)
      {
        println("drawing microslice: "+slice);
        for (int i=0;i<((Slice)slices.get(slice)).segments.size();i++)
        {
          Line segment=((Line)((Slice)slices.get(slice)).segments.get(i));
          output.beginShape();
  ...
  void scaleModel()
  {
    ...
    float[] scalingFactors= {
      ...
      scalingFactor=sort(scalingFactors)[0]*(1-margins);
    ...
    boundVertex=new Vertex(bound.x, bound.y, bound.z);
    for (int i=0;i<vertices.size();i++)
    {
      Vertex vertex=(Vertex)vertices.get(i);
  ...
  void sliceModel(float thickness, char axis)
  {
    ...
    boolean crossesPlane(Triangle triangle, char axis, float plane)
    {
      ...
      boolean above=( (triangleVertices[0].getAxis(axis) > plane) && (triangleVertices[1].getAxis(axis) > plane) && (triangleVertices[2].getAxis(axis) > plane));
      boolean below=((triangleVertices[0].getAxis(axis) < plane) && (triangleVertices[1].getAxis(axis) < plane) && (triangleVertices[2].getAxis(axis) < plane));
...
/*      vertex.point.x-=min.point.x;
      ...
      if(!above && !below)
      {
        print(plane+":  ");
        for(int i=0;i<3;i++)
          print((triangleVertices[i].getAxis(axis)+min.getAxis(axis)-(boundVertex.getAxis(axis)-(max.getAxis(axis)-min.getAxis(axis))/2))/scalingFactor+"  ");
        println();
      }
